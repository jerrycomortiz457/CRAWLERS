...
class Main extends CI_Controller {
	...
	{				
		...
		// $page = 0;
		$url = "https://www.careerbuilder.com/jobs-software-developer-software-engineer-in-san-jose,-ca,usa?radius=5&sort=date_desc";
		set_time_limit(0);
		...
		for($index = 0, $page = 1; $index < ceil($data['total_result_count']/$show_results_count);$index++, $page++){
			// $data['process'] = $index;
			$loop_url = "https://www.careerbuilder.com/jobs-software-developer-software-engineer-in-san-jose,ca,usa?page_number={$page}&radius=5&sort=date_desc";
			$get_html = file_get_html($loop_url);
		
		// 	//Scan every company
			for($index = 0, $result_number = 1; $index < count($get_html->find('a')); $index++, $result_number++){
				if($company_name->getattribute('data-gtm') == 'jrp-job-list|company-click|'.$result_number){
					$data['company_name'][] = $get_html->find('a')[$index]	->plaintext;
				}	
			}
		...
		// 	//Scan every jobtitle
				foreach($get_html->find('h2') as $job_name){
					if($job_name->getattribute('class') == 'job-title show-for-medium-up'){
						$data['job_title'][] = $job_name->plaintext;				
					}
				}			
	}
	...
	public function monster_crawl()
	{				
		//Variables with Target URL
		$show_results_count = 100;
		$url = "https://www.dice.com/jobs/advancedResult.html?for_one=software+developer+software+engineer&for_all=&for_exact=&for_none=&for_jt=&for_com=&for_loc=San+Jose%2C+CA&sort=date&limit=100&radius=El";
		set_time_limit(0);
		...
		for($index = 0, $page = 1; $index < ceil($data['total_result_count']/$show_results_count);$index++, $page++){
			// $data['process'] = $index;
			$loop_url = "https://www.dice.com/jobs/q-(software_OR_developer_OR_software_OR_engineer)-sort-date-limit-100-l-San_Jose%2C_CA-radius-El-startPage-1-limit-100-jobs";
			// $page = $index * $show_results_count;	
			$get_html = file_get_html($loop_url);
